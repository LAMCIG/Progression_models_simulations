...
relevant_cols = [col for col in df.columns if col.startswith(('lh_', 'rh_')) and '_thickness' in col]
relevant_cols += ["MMSE", "ADAS11", "ADAS13"]
df = df.replace([np.inf, -np.inf], np.nan)
df = df.dropna(subset=relevant_cols)

print("after drop na", df.shape)
subj_counts = df['PTID'].value_counts()
num_unique = (subj_counts == 1).sum()
print("one time PTID:", num_unique)

longitudinal_ids = subj_counts[subj_counts > 1].index
df = df[df['PTID'].isin(longitudinal_ids)].copy()
df = df.drop_duplicates(subset=["PTID", "Years_bl"])
print("after drop dupes", df.shape)

biomarker_names = [col for col in df.columns 
                   if col.startswith(('lh_', 'rh_')) and 
                   col.endswith('_thickness')]

print(biomarker_names)

X_obs = df[biomarker_names]
...
ids = df["PTID"].to_numpy()
dt = df["Years_bl"].to_numpy()
cog = df[["MMSE", "ADAS11", "ADAS13"]].to_numpy()
print("nans in cog:", np.isnan(cog).sum())
print("infs in cog:", np.isinf(cog).sum())

# Map DX to numeric (replacement for NHY)
dx_map = {"CN": 0, "MCI": 1, "Dementia": 2}
df["DX_numeric"] = df["DX"].map(dx_map)
dx = df["DX_numeric"].to_numpy()

# CDR-SB as continuous severity measure (replacement for NSD_STAGE)
cdrsb = df["CDRSB"].to_numpy()

# ADNI equivalents for PPMI's nhy / nsd (used in create_patient_list and validation)
nhy = dx
nsd = cdrsb

patient_df = df.copy()
patient_df["DX_numeric"] = dx
patient_df["CDRSB"] = cdrsb
grouped = patient_df.groupby("PTID")[["DX_numeric", "CDRSB"]].mean().dropna()

### INITIALIZE BETA FROM CLINICAL SCORES (bypass df: utility expects PPMI/synthetic columns)
# Pass 1D severity so that higher = worse. Utility does S = -z(cog), so pass -CDRSB.
severity_for_beta = -np.asarray(cdrsb, dtype=float)
initial_beta, pid_to_beta, _ = fit_mixedlm_beta_from_clinical(
    ids=ids,
    dt=dt,
    cog=severity_for_beta,
    t_max=t_max,
    verbose=True,
