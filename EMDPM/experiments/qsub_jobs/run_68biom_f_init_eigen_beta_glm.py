...
def build_severity_index(df):
    """
    Make a single severity outcome that *increases* with disease.
    MCATOT tends to *decrease* with worse disease, so we negate it.
    TD_score and PIGD_score increase with disease.
    """
...
def fit_mixedlm_beta_from_clinical(df, ids, dt, t_max, verbose=False, rng=None):
    """
    Mixed effects:  severity ~ dt  + (1 | id)
    Then beta_i ≈ random_intercept_i / slope_dt.
    Finally shift so min(beta)>=0 and clip to [0, t_max].
    Returns initial_beta aligned to np.unique(ids), a pid->beta dict, and the fit result.
    """
    if rng is None:
    ...
    for meth in ("bfgs", "nm"):
        try:
            ...
            if verbose:
                print(f"[MixedLM] method={meth} converged; llf={result.llf:.3f}")
            break
        except Exception as e:
            if verbose:
                print(f"[MixedLM] {meth} failed: {e}")
            result = None
...
out_path = os.path.join(out_dir, f"f_init_candidate_{current_candidate}_glm.npz")
